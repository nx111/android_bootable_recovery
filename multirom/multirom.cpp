...
/*
bool MultiROM::FilesystemSupportsSparseImage(const std::string& base)
{
	bool ret = false;
	char cmd[512];

	snprintf(cmd, sizeof(cmd), "make_ext4fs -s -l %dM \"%s/test.sparse.img\"", 96, base.c_str());
	if (system(cmd) == 0) {
		mkdir("/tmp/testsparse", 0777);
		snprintf(cmd, sizeof(cmd), "mount -o loop -t ext4 \"%s/test.sparse.img\" /tmp/testsparse", base.c_str());
		ret = system(cmd) == 0;
		system("umount -d /tmp/testsparse");
		rmdir("/tmp/testsparse");
	}
	else
		// Couldn't make_ext4fs
