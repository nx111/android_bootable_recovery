...
bool will_VendorBin_Be_Symlinked(void) {
	return (!PartitionManager.Is_Mounted_By_Path("/vendor") && TWFunc::Path_Exists("/system/vendor"));
}
...
static int Find_Firmware_Files(const string& Path, vector<string> *FileList) {
	int ret;
	DIR* d;
	struct dirent* de;
	string FileName;
...
void Parse_RC_File(const string& rc_file, vector<RC_Service>& RC_Services) {
	...
	vector<string> imports;     // file names of imports (we don't want to recurse while the current file is open)
	...
	// now that the file is closed, recurse any imports
...
vector<AdditionalService> Get_List_Of_Additional_Services(void) {
	...
	for (size_t i = 0; i < service_names.size(); ++i) {
		...
		if (service_names[i].length() > 12) {
			svc.Service_Name = service_names[i].substr(0, 12);
			svc.exists = false;
			services.push_back(svc);
		}
	}
	...
	for (vector<RC_Service>::iterator rc_iter = RC_Services.begin(); rc_iter != RC_Services.end(); ++rc_iter) {
		...
		// Check for vold_decrypt services and disable any that do have existing binaries
		for (vector<AdditionalService>::iterator iter = services.begin(); iter != services.end(); ++iter) {
			...
			if (prefix == "ven_" && will_VendorBin_Be_Symlinked()) {
				// /vendor is going to get symlinked to /system/vendor
				path = "/system" + path;
			}

			if (rc_iter->Service_Name == prefix + iter->Service_Name) {
				if (iter->VOLD_Service_Name.empty()) {
					if (!TWFunc::Path_Exists(path)) {
						iter->exists = false;
						LOGERROR("Service %s: Path does not exist %s\n", rc_iter->Service_Name.c_str(), path.c_str());
					} else {
...
int Vold_Decrypt_Core(const string& Password)
{
	...
	//~ VoldDecrypt_SymlinkManager SymlinkManager;
	//~ SymlinkManager.begin();
