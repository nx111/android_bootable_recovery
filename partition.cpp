...
bool TWPartition::Mount(bool Display_Error) {
	...
	else if(Is_ImageMount)
	{
		...
		LoopDevice = MultiROM::Create_LoopDevice(Primary_Block_Device);
		if (LoopDevice.empty())
		{
			if(Display_Error)
		...
		if(mount(LoopDevice.c_str(), Mount_Point.c_str(), Fstab_File_System.c_str(), 0, "") < 0)
		{
			MultiROM::Release_LoopDevice(LoopDevice, true);
			LoopDevice.clear();
			if(Display_Error)
		...
		LOGINFO("Loop mounted %s at %s using %s\n", Primary_Block_Device.c_str(), Mount_Point.c_str(), LoopDevice.c_str());
		mounted = 1;
	}
#endif //TARGET_RECOVERY_IS_MULTIROM
...
#ifdef TARGET_RECOVERY_IS_MULTIROM
			if(Is_ImageMount) {
				MultiROM::Release_LoopDevice(LoopDevice, false);
				LoopDevice.clear();
			}
#endif
			return true;
